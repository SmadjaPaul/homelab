...
variable "management_vm" {
  description = "Management VM configuration (Omni + Cloudflare Tunnel + Comet)"
  type = object({
    name   = string
    ocpus  = number
    memory = number
  ...
  default = {
    name   = "oci-hub"
    ocpus  = 1
    memory = 4 # Léger: Omni + Docker containers
    disk   = 50
  }
}

variable "k8s_nodes" {
  description = "Kubernetes Talos nodes - Free Tier: max 4 OCPU / 24GB total"
  type = list(object({
    name   = string
    ocpus  = number
    memory = number
  ...
  default = [
    {
      name   = "talos-cp-1" # Control Plane
      ocpus  = 1
      memory = 6
      disk   = 50
    },
    {
      name   = "talos-worker-1" # Worker principal (plus de RAM)
      ocpus  = 1
      memory = 8
      disk   = 64
    },
    {
      name   = "talos-worker-2" # Worker secondaire
      ocpus  = 1
      memory = 6
      disk   = 64
    }
...
variable "tailscale_auth_key" {
  description = "Tailscale authentication key for the subnet router (generate at https://login.tailscale.com/admin/settings/keys)"
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "cloudflare_tunnel_token" {
  description = "Cloudflare Tunnel token (get from: Cloudflare Zero Trust → Networks → Tunnels → Create tunnel → Docker)"
  type        = string
  default     = ""
  sensitive   = true
}
