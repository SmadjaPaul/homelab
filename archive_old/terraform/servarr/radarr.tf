resource "radarr_download_client_qbittorrent" "qbitty" {
  ...
  username                   = "smadja"
  password                   = data.doppler_secrets.servarr.map.QBITTY_PASSWORD
...
resource "radarr_download_client_sabnzbd" "sabnzbd" {
  ...
  api_key                    = data.doppler_secrets.servarr.map.SABNZBD_API_KEY
  remove_failed_downloads    = true
  remove_completed_downloads = true
}
...
resource "radarr_notification_gotify" "gotify" {
  ...
  name                             = "Gotify"
  server                           = "https://go.smadja.io"
  app_token                        = data.doppler_secrets.servarr.map.GOTIFY_RADARR_KEY
  priority                         = 8
}
