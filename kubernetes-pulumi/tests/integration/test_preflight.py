...
def test_cnpg_crds_readiness():
    ...
    errors = []
    if missing_crds:
        errors.append("Missing or Unestablished CRDs:\\n  - " + "\\n  - ".join(missing_crds))
    if invalid_labels:
        errors.append("CRDs missing Helm labels (will cause Server-Side conflicts):\\n  - " + "\\n  - ".join(invalid_labels))
...
def test_core_platform_crds_readiness():
    ...
    if missing_crds:
        pytest.fail("Missing or Unestablished Core CRDs:\\n  - " + "\\n  - ".join(missing_crds))
