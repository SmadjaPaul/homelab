...
def test_deny_all_policy_exists(test_case, k8s_available):
    ...
    assert res.returncode == 0, f"Deny-all NetworkPolicy {policy_name} not found in {test_case.namespace}"
...
def test_dns_policy_exists(test_case, k8s_available):
    ...
    assert res.returncode == 0, f"DNS allow NetworkPolicy {policy_name} not found in {test_case.namespace}"
        ...
        for policy in items:
            ...
            for rule in spec.get("egress", []):
                for peer in rule.get("to", []):
                    if peer.get("namespaceSelector", {}).get("matchLabels", {}).get("name") == isolated_ns:
                        pytest.fail(f"Namespace {test_case.namespace} should NOT reach {isolated_ns}")
