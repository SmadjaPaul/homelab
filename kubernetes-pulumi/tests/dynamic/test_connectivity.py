...
def test_pods_running(test_case, k8s_available):
    ...
    res = subprocess.run(
        ["kubectl", "get", "pods", "-n", test_case.namespace, "-l", f"app.kubernetes.io/name={test_case.name}", "-o", "json"],
    ...
    if not items:
        pytest.skip(f"No pods found with label app.kubernetes.io/name={test_case.name} in {test_case.namespace}")
    ...
    assert not failed_pods, f"Failed pods for {test_case.name}: {', '.join(failed_pods)}"
...
def test_service_exists(test_case, k8s_available):
    ...
    res = subprocess.run(
        ["kubectl", "get", "service", "-n", test_case.namespace, "-l", f"app.kubernetes.io/name={test_case.name}"],
        capture_output=True, text=True
    )
    assert res.returncode == 0, f"No service found with label app.kubernetes.io/name={test_case.name} in {test_case.namespace}"
...
def test_service_account_exists(test_case, k8s_available):
    ...
    assert res.returncode == 0, f"ServiceAccount {test_case.name} not found in {test_case.namespace}"
...
def test_servicemonitor_exists(test_case, k8s_available):
    ...
    assert res.returncode == 0, f"ServiceMonitor {test_case.name} not found in {test_case.namespace}"
