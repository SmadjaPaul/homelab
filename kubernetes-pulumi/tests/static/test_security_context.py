    ...
    for app in apps:
        ...
        for doc in yaml.safe_load_all(res.stdout):
            ...
            try:
                ...
                has_pvc = any(
                    "persistentVolumeClaim" in v for v in volumes if isinstance(v, dict)
                )
                ...
                if has_pvc:
                    ...
                    has_container_sc = any(
                        ...
                        and (
                            "runAsUser" in c["securityContext"]
                            or "runAsNonRoot" in c["securityContext"]
                        )
                        for c in containers
                    )
                    ...
                    if not (has_fsgroup or has_container_sc):
                        ...
                        errors.append(
                            f"App '{app_name}' ({kind}/{name}) mounts persistent storage but lacks a safe SecurityContext (missing fsGroup, runAsUser, or runAsNonRoot). Add to helm.values.podSecurityContext or helm.values.securityContext."
                        )
            except KeyError:
                pass
