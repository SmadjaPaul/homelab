    ...
    for app_config in apps:
        ...
        for doc in yaml.safe_load_all(res.stdout):
            ...
            if kind in [
                ...
                try:
                    if kind == "CronJob":
                        template_spec = doc["spec"]["jobTemplate"]["spec"]["template"][
                            "spec"
                        ]
                    elif kind == "Pod":
                    ...
                    for c in containers:
                        if isinstance(c, dict):
                            ...
                            for e_from in c.get("envFrom", []):
                                if (
                                    "secretRef" in e_from
                                    and "name" in e_from["secretRef"]
                                ):
                                    chart_referenced_secrets.add(
                                        e_from["secretRef"]["name"]
                                    )
                            ...
                            for e in c.get("env", []):
                                if (
                                    ...
                                    chart_referenced_secrets.add(
                                        e["valueFrom"]["secretKeyRef"].get("name")
                                    )
        ...
        for req_secret in chart_referenced_secrets:
            ...
            if (
                not req_secret
                or req_secret.endswith("token")
                or "helm" in req_secret
                or req_secret in IGNORE_SECRETS
            ):
                continue

            if (
                req_secret not in chart_created_secrets
                and req_secret not in provisioned_secrets
            ):
                errors.append(
                    f"App '{app_name}' references secret '{req_secret}' but it is NOT created by the Helm chart and NOT provisioned in apps.yaml."
                )

        # 4. Check for DockerHub secret requirement
        if (
            "dockerhub-secret" not in chart_referenced_secrets
            and "dockerhub-secret" not in provisioned_secrets
        ):
        ...
        for doc in yaml.safe_load_all(res.stdout):
            if (
                ...
                or doc.get("kind") not in ["Deployment", "StatefulSet", "DaemonSet"]
            ):
                continue
            ...
            for c in containers:
                for e in c.get("env") or []:
                    ...
                    for pattern in EXTERNAL_DB_PATTERNS:
                        if pattern in val:
                            errors.append(
                                f"App '{app_name}' has environment variable '{e.get('name')}' pointing to external DB ({val}). Should it be local?"
                            )
