...
def check_stale_pvcs():
    ...
    try:
        ...
        output = run_cmd("kubectl get pvc -A -o json")
    except subprocess.CalledProcessError:
...
def main():
    parser = argparse.ArgumentParser(
        description="Pre-flight checks for Kubernetes deployment"
    )
    parser.add_argument(
        "--check-pvc", action="store_true", help="Check stale PVCs only"
    )
    parser.add_argument("--check-crd", action="store_true", help="Check CRDs only")
    parser.add_argument(
        "--fix", action="store_true", help="Auto-fix issues (delete stale PVCs)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--cluster", default="oci", help="Cluster context (default: oci)"
    )
    ...
    if stale_pvcs:
        ...
        print("   Or run with --dry-run to see what would be deleted")
        sys.exit(1)
