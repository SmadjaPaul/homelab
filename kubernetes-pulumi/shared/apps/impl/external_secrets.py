...
class ExternalSecretsApp(GenericHelmApp):
    ...
    def deploy_components(self, provider: k8s.Provider, config: Dict[str, Any], opts: Optional[pulumi.ResourceOptions] = None) -> Dict[str, Any]:
        ...
        waiter = Waiter("webhook-waiter", opts=pulumi.ResourceOptions(depends_on=[release]))
        ...
        print(f"  [ExternalSecretsApp] Creating ClusterSecretStore doppler (Project: {doppler_project}, Config: {doppler_config})...")
        css = k8s.apiextensions.CustomResource(
            ...
            opts=pulumi.ResourceOptions(provider=provider, depends_on=[waiter]),
        )
        print(f"  [ExternalSecretsApp] Created ClusterSecretStore doppler")
