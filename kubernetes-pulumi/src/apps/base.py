...
class BaseApp(ABC):
    ...
    def deploy(
        ...
        Handles:
        - Namespace creation
        - Network policies (if enabled in test_config)

        Calls deploy_components() for app-specific resources.
        """
        ns = k8s.core.v1.Namespace(
            self._model.namespace,
            metadata={"name": self._model.namespace},
            opts=pulumi.ResourceOptions(provider=provider),
        )

        result = {"namespace": ns}

        components = self.deploy_components(provider, config)
...
class NetworkPolicyBuilder:
    ...
    def build(self, app: BaseApp) -> list:
        ...
        # 2. Allow egress to DNS (kube-system)
