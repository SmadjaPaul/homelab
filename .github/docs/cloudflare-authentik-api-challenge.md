...
La règle est gérée par `terraform/cloudflare/security.tf` (ressource `cloudflare_ruleset.authentik_api_skip_challenge`).
...
- `terraform/cloudflare/security.tf` – définition Terraform de la règle
