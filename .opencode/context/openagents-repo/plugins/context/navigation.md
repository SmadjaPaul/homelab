<!-- Context: openagents-repo/navigation | Priority: critical | Version: 1.0 | Updated: 2026-02-15 -->

# Context Plugin

**Purpose**: Documentation for the OpenAgents Context plugin system

---

## Structure

```
context/
├── navigation.md (this file)
├── context-overview.md
├── architecture/
├── capabilities/
├── concepts/
├── guides/
├── lookup/
└── reference/
```

---

## Quick Routes

| Task | Path |
|------|------|
| **Plugin overview** | `context-overview.md` |
| **Architecture** | `architecture/navigation.md` |
| **Capabilities** | `capabilities/navigation.md` |
| **Create plugins** | `guides/creating-plugins.md` |
| **Plugin structure** | `lookup/plugin-structure.md` |

---

## By Type

**Architecture** → Plugin system design
**Capabilities** → What plugins can do
**Concepts** → Core plugin concepts
**Guides** → How to build plugins
**Reference** → Best practices and patterns

---

## Related Context

- **Plugins** → `../navigation.md`
- **OpenAgents** → `../../navigation.md`
- **Concepts** → `../../concepts/navigation.md`
