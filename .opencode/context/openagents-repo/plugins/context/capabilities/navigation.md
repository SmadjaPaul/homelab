<!-- Context: openagents-repo/navigation | Priority: critical | Version: 1.0 | Updated: 2026-02-15 -->

# Context Plugin Capabilities

**Purpose**: What the plugin system can do

---

## Structure

```
capabilities/
├── navigation.md (this file)
├── agents.md
├── events.md
├── events_skills.md
└── tools.md
```

---

## Quick Routes

| Task | Path |
|------|------|
| **Agent capabilities** | `agents.md` |
| **Event system** | `events.md` |
| **Events & skills** | `events_skills.md` |
| **Tool capabilities** | `tools.md` |

---

## By Type

**Agents** → Agent registration and management
**Events** → Event system and hooks
**Tools** → Tool registration and usage

---

## Related Context

- **Context Plugin** → `../navigation.md`
- **Architecture** → `../architecture/navigation.md`
- **Guides** → `../guides/navigation.md`
