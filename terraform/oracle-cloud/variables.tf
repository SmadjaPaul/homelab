...
# -----------------------------------------------------------------------------
# Talos Linux (K8s nodes boot with Talos OS)
# -----------------------------------------------------------------------------
# Use the image generated by Omni UI (pre-configured with credentials). Download from Omni,
# import to OCI as custom image, then set talos_image_id. See README and:
# https://blog.zwindler.fr/2025/01/04/sideros-omni-talos-oracle-cloud/

variable "talos_image_id" {
  description = "OCID of the custom Talos image imported from Omni (Omni UI → download Oracle image → import to OCI → use this OCID). K8s nodes boot with Talos and enroll in Omni at first boot."
  type        = string
  default     = ""
}
...
# With Cloudflare Tunnel, traffic reaches the VM outbound only (tunnel). Opening 80/443 is
# optional — set to false for "tunnel-only" to reduce attack surface (recommended).
# See: https://dev.to/yoursunny/how-to-host-a-website-in-oracle-cloud-free-tier-5hca
variable "allow_public_http_https" {
  description = "Allow ingress TCP 80/443 on Security List and UFW. Set false for tunnel-only (recommended)."
  type        = bool
  default     = false
}
...
variable "vault_secret_authentik_token" {
  description = "Authentik API token (bootstrap, for initial OAuth2 provider creation, stored in OCI Vault)"
  type        = string
  default     = ""
  sensitive   = true
}

variable "vault_secret_authentik_oauth2_client_id" {
  description = "Authentik OAuth2 client_id (CI automation provider, stored in OCI Vault). Content populated after first terraform apply."
  type        = string
  default     = ""
  sensitive   = false
}

variable "vault_secret_authentik_oauth2_client_secret" {
  description = "Authentik OAuth2 client_secret (CI automation provider, stored in OCI Vault). DEPRECATED: Use private_key_jwt instead."
  type        = string
  default     = ""
  sensitive   = true
}

variable "vault_secret_authentik_private_key_pem" {
  description = "Authentik RSA private key PEM (for private_key_jwt authentication, stored in OCI Vault). Generated by GitHub Actions workflow."
  type        = string
  default     = ""
  sensitive   = true
}
