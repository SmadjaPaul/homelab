...
output "service_records" {
  value = { for k, v in cloudflare_dns_record.homelab_services : k => { name = v.name, type = v.type } }
}

output "tunnel_cname_records" {
  value = { for k, v in cloudflare_dns_record.tunnel_cname : k => { name = v.name, content = v.content } }
}
